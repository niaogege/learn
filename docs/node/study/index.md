...
- V8：Google 开源的高性能 JavaScript 引擎，使用 C++ 开发，并且应用于谷歌浏览器。如果您感兴趣想学习更多的 V8 引擎知识
...
非阻塞 I/O 又称异步式 I/O，是指当线程遇到 I/O 操作时，不会以阻塞方式等待 I/O 操作完成或数据返回，而只是将 I/O 请求转发给操作系统，继续执行下一条指令。

Node.js 避免了由于需要等待输入或者输出（数据库、文件系统、Web 服务器...）响应而造成的 CPU 时间损失，这得益于 **Libuv** 强大的异步 I/O。

### 事件驱动

最常见的形式就是回调，触发一次事件，然后通过回调来接收一些处理
...
node.js 中的事件驱动机制指的是 node.js 在运行时会产生各种事件,我们可以通过代码对这些事件进行监听和响应,从而驱动整个程序的运行。

**node.js 采用单线程运行,通过非阻塞 I/O 和事件驱动机制,让一个线程可以高效处理并发事件。当某个 I/O 操作完成或者消息到达时,node.js 会触发一个事件。我们可以在代码中绑定事件的回调函数,当这个事件触发时,对应的回调函数就会被调用。**
...
- 单线程的缺点
  ...
  3)大量计算占用 CPU, 导致无法继续调用异步 I / O 后续也推出了**child_process 和 cluster**模块较好地缓解了以上缺点
