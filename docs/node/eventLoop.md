...
Node 中的 Event Loop 和浏览器中的是完全不相同的东西。 Node 的 Event Loop 分为 6 个阶段，它们会按照顺序反复运行。每当进入某一个阶段的时候，都会从对应的回调队列中取出函数去执行。当**队列为空或者执行的回调函数数量到达系统设定的阈值**，就会进入下一阶段。

- 1.**Timers（计时器阶段）**: 这个阶段执行 由 timer（setTimeout、setInterval）创建的定时器回调函数

- 2.**I/O 事件回调阶段**： 执行延迟到下一个循环迭代的 I/O 回调，即执行上一轮循环中未被执行的 I/O 回调，如文件读写/网络请求等

- 3.**idle 和 prepare 闲置阶段**：仅供 node 内部使用

- 4.**进入 poll 轮询阶段**，检索新的 I/O 事件，执行与 I/O 相关的回调，如 TCP 链接/http 请求
...
- 5.**check 查询类型**：执行所有类型为 check 的 宏任务，然后执行所有的 微任务，setImmediate 回调函数会在此阶段执行

- 6.**关闭回调 close callbacks**：执行与关闭操作相关的回调函数，然后执行所有的 微任务
...
#### 精简 Node.js 中的事件循环机制与浏览器中的差异

1.微任务执行时机不同： Nodejs 中微任务的执行时机是每个阶段的宏任务(宏任务队列)结束之后执行，而不是当前宏任务执行结束后
